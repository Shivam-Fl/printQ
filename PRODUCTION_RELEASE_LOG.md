...
| Resend | A sending-only production API key was created and saved as a protected Render `RESEND_API_KEY` variable without printing it. Domain `mail.printqs.com` added. On 2026-09-26 the in-app tab showed a login page and a Chrome domain-page attempt timed out; no authenticated sender status could be read. | Plan/usage cap has not been audited. Last observed sender state was **pending**; DNS resolution alone is not verification, so reset email is not yet production-ready. |
