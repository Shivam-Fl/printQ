...
The external Chrome Render session was authenticated, but service-page inspection repeatedly timed out. Authenticated Render CLI and GitHub CLI supplied the read-only inventory in [PHASE_0_2_AUDIT.md](outputs/printqs-release-audit-20260926/PHASE_0_2_AUDIT.md). Production still serves live `d9319e8` (`dep-daqjk73l550s73bp7u8g`); `ff9f99f` failed and is not live. Free production PostgreSQL expires **2026-10-07**. A custom-format database baseline backup was created outside Git, its structure verified, and its ACL restricted to the Windows owner/SYSTEM/Administrators; a restore drill is still open. Production DB has 12 draft/unpublished shops, 0 active campuses, 8 student records and 83 jobs. **The old live public shop API nevertheless lists all 12 draft shops (8 obvious test/demo/QA), all offline**; this is an observed launch blocker, not a source-only inference.

Render `printq-web` was `autoDeployTrigger=commit`, so a GitHub failed check could still produce a deploy attempt. It is now **`off`**, verified through Render CLI, with no new deploy triggered. `render.yaml` now mirrors this exact-SHA manual promotion policy on PR #25; only `render deploys create srv-dafems740ujc73b080e0 --commit <verified-sha> --wait` should promote after development QA. The PR head's full CI, secret scan, gate, verification, QA, and Vercel checks passed after the change; GitHub branch protections remain active. Phase 1 is still partial until hosted development QA and release-source enforcement are complete. Phase 2 remains partial: the development project/bucket exist, but no hosted dev API/DB/Redis/WIF deployment exists; the Render workspace's single free PostgreSQL/Key Value slots are already occupied by production-named resources. No paid development infrastructure was created or requested.
...
| Active fix PR | [#25](https://github.com/Shivam-Fl/printQ/pull/25), branch `codex/render-prod-env-fix`. Firebase/DB fix SHA `9233a44ff5dcd60afe3f827c105062f1b79374d8`; GCS/Resend/log commit `6558123`; admin-recovery commit `3cb5b2a`. | On 2026-09-26, all seven displayed checks at `3cb5b2a` passed: CI/check, secret scan, gate, verify, QA, Vercel and Preview Comments. Recheck at the current PR head; merge only when required runtime variables and email are verified. |
...
| --- | --- | --- |
| Render web `printq-web` | Free, combined API/conversion/worker process, auto-deploys `master`. | 512 MB / 0.1 CPU, sleeps after idle, ephemeral filesystem; cannot claim reliable real-order capacity. [Render expressly says free instances are not for production](https://render.com/docs/free). |
