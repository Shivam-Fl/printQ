...
Last updated: 2026-09-23 (Asia/Kolkata)

## Current phase

Phases 0–7 remain implemented in stacked, unmerged candidates and Phase 8 Windows-agent hardening is in PR #19. Its required CI and secret scan passed at `39568e3`. The separate SDLC QA workflow had a missing-report failure on skipped runs; a scoped correction is now local and awaits CI. Phase 9 pay-at-shop checkout, staff cash/merchant-UPI confirmation, verified-print commission entries, support refund credits, weekly statement arithmetic, and queue override warning are implemented locally. The required full PostgreSQL/Redis simulator has been rewritten and awaits GitHub CI. Weekly Razorpay TEST mandate creation, notice delivery, exact debit dispatch, and signed settlement are not implemented or tested yet. No Cloud Run service or launch candidate has been deployed.
...
| Active pay-at-shop branch / PR | `codex/pay-at-shop-ledger` / not yet opened (stacked on PR #19; local implementation and regression suite active) |
...
| Pay-at-shop PostgreSQL/Redis simulator and migrations | Pending CI | Local Docker Desktop remains unusable. The rewritten simulator and new migrations must pass GitHub disposable PostgreSQL/Redis CI on the exact pushed commit before any development deploy. |
...
| Development PostgreSQL/Redis/GCS | Not yet provisioned | Cloud Run services page showed zero development services on 2026-09-23; development billing status could not be independently verified. Create distinct durable resources, private GCS, Artifact Registry, runtime/deploy identities, workload identity federation, and isolated Redis; never reuse production resources. |
...
Open the stacked pay-at-shop PR and require its disposable PostgreSQL/Redis migration and full simulator to pass. Finish TEST recurring collection and signed settlement against Razorpay's approved on-demand API. Provision only isolated development services and deploy the exact passing SHA through the development workflow; collect live browser, provider TEST, storage, printer-simulator, and deletion evidence. Then execute the full dated release QA matrix, fix failures through PRs, and deploy only that same accepted SHA to production. The separately required immediate confirmation still applies before any live mandate, key enablement, or real shop debit.
