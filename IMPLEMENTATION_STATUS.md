...
| Active Firebase outbox branch / PR | `codex/firebase-outbox` / PR pending (stacked on the App Check candidate; unmerged and undeployed) |
...
| Development PostgreSQL/Redis/GCS | Not yet provisioned | The Google Cloud account is signed in and development billing was linked on 2026-09-18. Cloud Run Admin API activation is blocked by Google’s required account-administrator verification prompt. After the owner completes that external verification, create distinct durable resources, a private GCS bucket with uniform bucket-level access and public access prevention, Artifact Registry, runtime/deploy identities, and workload identity federation; do not create or reuse production resources. |
