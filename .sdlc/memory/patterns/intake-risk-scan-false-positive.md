...
**Fix:** `strippedForRisk()` (`.sdlc/bin/lib/triage.js`) removes headed sections whose heading
matches out-of-scope / non-goals / "explicitly excluded" / notes / background / prior-art
before the keyword scan runs. The title is never stripped — a risky change can't be hidden
under such a heading. Applies to any future keyword-based gate over free-form issue/PR text in
this repo: strip declared-negative sections before scanning, always scan the title.

**Related, same PR:** re-running intake on an issue already in `needs-human` failed with
`illegal transition needs-human -> needs-human`. `.sdlc/bin/lib/ledger.js` now treats landing
on the state already held as a no-op instead of an illegal transition — a retry or a replayed
